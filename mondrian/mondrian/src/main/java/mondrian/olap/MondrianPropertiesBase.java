...
public abstract class MondrianPropertiesBase extends TriggerableProperties {
    ...
    public static final String buildNums = "v:1.0.0.4096_2-LMX ";
    ...
    public void populate() {
        ...
        Properties monrianpros = load2(file.getAbsolutePath());        
        ...
        keys_m.hasMoreElements();)
        {
            String key = (String) keys_m.nextElement();
            String value = monrianpros.getProperty(key);
            if (key.startsWith("mondrian.")) {
                // NOTE: the super allows us to bybase calling triggers
                // Is this the correct behavior?
                if (LOGGER.isDebugEnabled()) {
                    ...
                    LOGGER.debug(buildNums +"Properties loaded: key=" + key + ", value=" + value);
                }
                super.setProperty(key, value);
                count++;
            }
        ...
        // LOGGER.info(buildNums+
        //         "Mondrian: try load OptimizePredicates " + MondrianProperties.instance().OptimizePredicates.get() + " MondrianProperties  ");            
    }
    ...
    private Properties load2(String fileName)
    {

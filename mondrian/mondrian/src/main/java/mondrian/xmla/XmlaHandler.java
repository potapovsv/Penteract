...
//                        if (true) {
                ...
                if (queryPart instanceof DrillThrough) {
                    ...
                    if (LOGGER.isDebugEnabled()) {
                        long end = System.currentTimeMillis();
                        LOGGER.debug("XmlaHandler.execute: time = executeQuery(request)" + (end - start));
                        LOGGER.debug("XmlaHandler.execute: executeQuery(request)" + Util.printMemory());
                        // LOGGER.debug("XmlaHandler.process: sizeresponse " + response.getWriter(). );
                    }                    
                }
            }
            ...
            try {
                switch (content) {
                    ...
                    case DataIncludeDefaultSlicer:
                        if (result != null) {
                            if (LOGGER.isDebugEnabled()) {
                                LOGGER.debug("Trouble space. getStackSize:= "  );
                            }
        ...
        throws XmlaException, OlapException
    {
        ...
        try {
            ...
            try {
                ...
                if (LOGGER.isDebugEnabled()) {
                    long end = System.currentTimeMillis();
                    LOGGER.debug("XmlaHandler.QueryResult: time = " + (end - start));
                    LOGGER.debug("XmlaHandler.StackTrace: " + Arrays.toString(stackTrace));
                    // LOGGER.debug("XmlaHandler.process: sizeresponse " + response.getWriter(). );
                }
...
*/
            ...
            int ordinal)
        {
            ...
            for(String propertyName: this.queryCellPropertyNames){
                ...
                if(propertyName.equals("FONT_FLAGS") || propertyName.equals("FORMAT_STRING")) {
                    // надо подумать пропускать только FORMAT_STRING = STANDART <FormatString>Standard</FormatString>
                    continue;
                }
